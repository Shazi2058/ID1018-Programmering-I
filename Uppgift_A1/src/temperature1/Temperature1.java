...
of weeks. The measurements are taken regularly ó the same number of
...
public class Temperature1 {
	...
	public static void main (String[] args)
	{
		...
		for (int week=1; week <= nofWeeks; week++)
		{
			double min = t[week][1]; // Picks the initial value of measurement from the week  
			double max = t[week][1]; // Picks the initial value of measurement from the week  
			double sum = 0;
			double avg = 0;
			for (int measurement = 1; measurement <= nofMeasurementsPerWeek; measurement++) {
		...
		//Calculation maximum
		for (int i = 2; i < maxT.length; i++) 
		{
		if (maxT[i] > maxTemp)
			maxTemp = maxT[i];
		}
		//Calculation Whole Sum
